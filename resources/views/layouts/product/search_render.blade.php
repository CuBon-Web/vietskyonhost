...
@endphp
<div class="w-100">
    <a href="{{route('detailProduct',['cate'=>$item['cate_slug'],'type'=>$item['type_slug'] ? $item['type_slug'] : 'type','id'=>$item['slug']])}}" title="{{$item->name}}" class="d-flex align-items-start w-100 py-2 result-item border-bottom align-item js-link">
        ...
        <div class="result-item_detail px-2">
            ...
            @if ($item->price > 0)
                ...
                <s class="result-item_compare-price font-weight-normal ms-2 d-inline js-compare-price">{{number_format($item->price)}}₫</s>
                @endif
            @elseif($item->price == 0 && $item->discount> 0)
            <div class="result-item_price overflow-hidden font-weight-bold d-inline js-price">{{number_format($item->discount)}}₫</div>
            @else
